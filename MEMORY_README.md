...
It is designed around a simple Claude Code session rhythm:

- start each meaningful session with `/resume`
- end each meaningful session with `/checkpoint`

`/resume` reconstructs the current project state from the memory files before work begins.
`/checkpoint` is the single maintenance gateway for the repository:
...
Then, at the beginning of the session:

```text
/resume
```

Use `/resume` to load the current objective, architecture context, blockers, and next actions from:

- `CLAUDE.md`
- `context/project-overview.md`
- `context/worklog.md`
- latest `context/session-summaries/<date>.md`
- `context/architecture.md` and `context/decisions.md` when relevant
- `graphify-out/GRAPH_REPORT.md` if present

During the session:

- use normal Claude Code prompts for implementation, debugging, documentation, and review
- use `/decision` for durable architectural or product decisions
- use `/ingest` to turn source material from `raw/` into curated `wiki/` knowledge
...
Use `/checkpoint` before stopping so the next `/resume` starts from accurate memory.
...
Do not blindly overwrite an existing project `README.md`; keep this scaffold guide as `MEMORY_README.md` or merge the relevant instructions into the project's real README.
...
- run `/resume`
- check the current objective, blockers, and next actions
- fix stale placeholder memory before relying on it
...
- `/resume` reconstructs the current objective, blockers, and next actions
...
- `/resume` reconstructs project state at the beginning of a Claude Code session.
